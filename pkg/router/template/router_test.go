...
func TestCreateServiceAliasConfig(t *testing.T) {
	...
	httpRequestHeadersList := []HTTPHeader{{Name: "Accept", Value: "'text/plain,text/html'", Action: "Set"}, {Name: "x-client", Value: `'"abc"\ '\''def'\'''`, Action: "Set"}, {Name: "Accept-Encoding", Action: "Delete"}}
	...
	if config.Host != route.Spec.Host || config.Path != route.Spec.Path || !compareTLS(route, config, t) ||
		...
		!cmp.Equal(config.HTTPResponseHeaders, httpResponseHeadersList) || !cmp.Equal(config.HTTPRequestHeaders, httpRequestHeadersList) || config.ActiveServiceUnits != 0 {
